...
def main():
	...
	highscore, best_sol = run_logic(response, highscore, runs+1, bags, recycle)
	print(f'Highscore: {highscore}')
	print(best_sol.bagPrice, best_sol.bagType, best_sol.recycleRefundChoice, best_sol.refundAmount, best_sol.orders)
	highscore, best_sol = run_logic(response, highscore, 100, [best_sol.bagType], [best_sol.recycleRefundChoice])
	store_data(highscore, best_sol.toJSON())

def run_logic(response, highscore, runs, bags, recycle):
	for bag in bags:
		for rec in recycle:
				for iteration in range(1,runs):
					try:
						...
						solution = solver.Solve(bag_type, days, rec)
